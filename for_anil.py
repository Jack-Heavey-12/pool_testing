	...
	for i in range(len(cascades)):
			for v in node_list:
				...
				if not val:
					# The node is not in the connected component, any set with this node in it is valid:
					#F_vi = [S for S in set_list if v in S]
					F_vi = [S for S in set_list if any(x in cascades[i] for x in S)] 
...
def read_graph(name):
	if name == 'test_graph':
		...
		for line in lines:
			lst.append(line.strip())
		network.close()
		H_prime = nx.parse_edgelist(lst[1000:2000])
		G = H_prime.subgraph(max(nx.connected_components(H_prime))).copy()
	elif name == 'random':
...
if __name__ == "__main__":
	...
	fl = .33
	with open('test_cascades/uva_post_500_0.33.pkl', 'rb') as f:
	...
	with open('anil_results.csv', 'a') as f:
		f.write(f'\n{len(graph)},{len(cascade_list)},{fl},{random_clearances}, Y, {len(set_list)}, {n_p},{budget}, Random Sets')
